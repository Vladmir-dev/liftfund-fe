...
<template>
  <div class="min-h-screen bg-slate-50/50 text-slate-800 font-sans text-left" @click="closeDropdowns">
    ...
    <main class="max-w-7xl mx-auto px-4 py-12">
      ...
      <div class="border-t border-slate-100 pt-12 mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <div class="flex items-center gap-4">
          <button v-if="selectedCategory" @click="selectedCategory = null" class="text-xs font-bold text-slate-400 hover:text-slate-600 underline">
            Clear Category Filter
          </button>
          
          <div class="relative w-full max-w-xs">
            <input type="text" v-model="searchQuery" placeholder="Search inside category..." class="w-full pl-9 pr-4 py-2 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-600 text-xs font-medium" />
      ...
      <div v-if="filteredFundraisers.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        <div v-for="fund in filteredFundraisers" :key="fund.id" class="flex flex-col bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-sm hover:shadow-lg transition-all group">
          <div class="aspect-video relative overflow-hidden bg-slate-100">
            ...
            <span class="absolute top-3 left-3 bg-white/90 backdrop-blur-sm text-blue-700 text-[10px] font-extrabold uppercase px-2.5 py-1 rounded-full border border-blue-50">
              {{ fund.category }}
            </span>
          </div>
          <div class="p-6 flex-1 flex flex-col justify-between">
            <div>
              ...
              <h3 class="font-bold text-slate-800 group-hover:text-blue-600 transition-colors mb-2 leading-snug line-clamp-1">{{ fund.title }}</h3>
            ...
            <div class="mt-4 pt-4 border-t border-slate-50">
              ...
              <div class="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
                <div class="h-full bg-blue-600 rounded-full" :style="{ width: getProgressPercent(fund.raisedAmount, fund.targetAmount) + '%' }"></div>
              </div>
              
              <button @click="openDonateModal(fund.id)" class="w-full mt-4 bg-slate-50 hover:bg-blue-600 hover:text-white text-blue-600 text-xs font-bold py-2.5 rounded-xl transition-all border border-blue-50">
                Donate Now
              </button>
            </div>
          </div>
    ...
    <div v-if="activeFundraiserId" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 backdrop-blur-sm" @click.self="closeDonateModal">
      <div class="bg-white rounded-3xl w-full max-w-md p-6 border border-slate-100 shadow-2xl animate-scale text-left relative">
        ...
        <div v-else>
          ...
          <form @submit.prevent="handleDonate" class="flex flex-col gap-4">
            ...
            <button type="submit" :disabled="isDonating" class="w-full bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold py-3 rounded-xl transition-all flex items-center justify-center gap-2">
