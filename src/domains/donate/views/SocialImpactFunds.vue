...
<template>
  <div class="min-h-screen bg-white text-slate-800 font-sans text-left animate-fade-in" @click="closeDropdowns">
    ...
    <main class="max-w-7xl mx-auto px-4 py-16">
      ...
      <div class="text-center max-w-4xl mx-auto mb-12">
        ...
        <p class="text-slate-500 text-sm sm:text-base leading-relaxed max-w-3xl mx-auto">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-10 mb-20">
        <div v-for="fund in funds" :key="fund.id" @click="openDonateModal(fund.id)" class="group bg-white rounded-3xl border border-slate-100/80 overflow-hidden shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between cursor-pointer">
          <div>
            ...
            <div class="p-6">
              <h3 class="font-extrabold text-slate-900 text-xl mb-1 group-hover:text-blue-600 transition-colors">{{ fund.name }}</h3>
              <span class="text-xs font-semibold text-slate-400 block mb-3">{{ fund.category }}</span>
              <p class="text-slate-500 text-xs sm:text-sm leading-relaxed">{{ fund.description }}</p>
            </div>
          </div>
          ...
          <div class="p-6 pt-0">
            ...
            <div class="p-4 bg-slate-50 rounded-2xl border border-slate-100 mb-4 text-xs font-bold text-slate-700">
              ...
              <div class="w-full h-2 bg-slate-200 rounded-full overflow-hidden">
                <div class="h-full bg-blue-600 rounded-full transition-all duration-500" :style="{ width: getProgressPercent(fund.raisedAmount, fund.targetAmount) + '%' }"></div>
              </div>
            </div>
            <button class="w-full bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold py-3.5 rounded-xl transition-all shadow-md shadow-blue-100 text-center cursor-pointer">
              Donate to {{ fund.name }}
            </button>
          </div>
        </div>
    ...
    <!-- Ways to Give Dark Navy Section -->
    <section class="bg-[#1e3a5f] text-white py-16 text-center border-t border-slate-800">
      <div class="max-w-4xl mx-auto px-4">
        <h2 class="text-3xl sm:text-4xl font-black mb-4">Ways to Give</h2>
        <p class="text-blue-100 text-sm sm:text-base max-w-xl mx-auto mb-8 leading-relaxed">
          We accept tax-deductible donations via checks, crypto, donor-advised funds (DAFs), and wire or ACH transfers.
        </p>
        <button @click="openDonateModal('fund-flood')" class="bg-white hover:bg-blue-50 text-[#1e3a5f] font-bold px-8 py-3.5 rounded-full shadow-lg transition-all text-sm cursor-pointer">
          Make a donation
        </button>
      </div>
    </section>
    ...
    <div v-if="activeFundId" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 backdrop-blur-sm" @click.self="closeDonateModal">
        ...
        <div v-else>
          ...
          <form @submit.prevent="handleDonate" class="flex flex-col gap-4">
            <div class="flex flex-col">
              ...
              <input type="number" v-model="donationAmount" min="5" required class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-600 text-sm" />
            </div>
            <button type="submit" :disabled="isDonating" class="w-full bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold py-3 rounded-xl transition-all flex items-center justify-center gap-2 cursor-pointer">
