...
const handleRequestWithdrawal = async () => {
  ...
  try {
    ...
    Notify.failure(submitError.value)
  } finally {
...
<template>
  <div class="flex flex-col gap-6 animate-fade-in text-left">
    ...
    <!-- Balance Banner -->
    <div
      class="bg-gradient-to-r from-[#edfce0] via-emerald-50 to-white border border-[#bbf770] rounded-3xl p-6 shadow-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
      <div>
        <span class="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-1">Available Campaign
          Balance</span>
    ...
    <div class="flex flex-col gap-4 mt-2">
      <div class="flex items-center justify-between">
        ...
        <button @click="fetchWithdrawals"
          class="text-xs font-bold text-[#024731] hover:underline cursor-pointer">Refresh</button>
      </div>
      ...
      <div v-else-if="withdrawals.length > 0" class="flex flex-col gap-3">
        <div v-for="w in withdrawals" :key="w.id"
          ...
          <div class="flex items-center gap-4">
            <div
              class="w-10 h-10 rounded-xl bg-emerald-50 text-[#024731] flex items-center justify-center font-bold text-base shrink-0">
            ...
            <div>
              <div class="flex items-center gap-2 mb-0.5">
                ...
                <span class="text-[10px] font-extrabold uppercase px-2.5 py-0.5 rounded-full border"
                  :class="formatStatus(w.status).class">
                  {{ formatStatus(w.status).label }}
                </span>
              </div>
              <p class="text-slate-400 text-xs font-medium">
                {{ typeof w.notes === 'string' ? w.notes : (w.notes?.Valid ? w.notes.String : 'Direct Transfer') }}
                &bull; {{ new Date(w.createdAt).toLocaleDateString() }}
              </p>
            </div>
          </div>
      ...
      <!-- Empty State -->
      <div v-else
        class="bg-white border border-dashed border-slate-200 rounded-3xl p-10 text-center flex flex-col items-center justify-center">
        ...
        <p class="text-slate-400 text-xs font-medium max-w-sm leading-relaxed mb-4">
          When you are ready to withdraw raised funds, submit a payout request here to transfer money to your phone or
          bank.
        </p>
        <button @click="openWithdrawModal"
    ...
    <div v-if="showWithdrawModal"
      ...
      <div
        class="bg-white rounded-3xl w-full max-w-md p-6 border border-slate-100 shadow-2xl relative animate-in fade-in duration-200">
        ...
        <div v-if="submitSuccess" class="py-8 text-center">
          <div
            class="w-14 h-14 rounded-full bg-[#edfce0] border border-[#bbf770] flex items-center justify-center text-3xl mb-3 text-[#02a95c] mx-auto">
          ...
          <p class="text-slate-500 text-xs font-semibold leading-relaxed">
            Your request for {{ currency }} {{ Number(withdrawAmount).toLocaleString() }} has been submitted and is
            being processed.
          </p>
        </div>
        ...
        <div v-else>
          ...
          <div v-if="submitError"
            class="mb-4 p-3 rounded-xl bg-rose-50 border border-rose-200 text-rose-700 text-xs font-semibold">
            {{ submitError }}
          </div>
          ...
          <form @submit.prevent="handleRequestWithdrawal" class="flex flex-col gap-4">
            ...
            <div>
              ...
              <div
                class="relative rounded-xl border border-slate-200 overflow-hidden focus-within:ring-2 focus-within:ring-[#024731]">
                <span class="absolute left-3 top-3 text-xs font-bold text-slate-500">{{ currency }}</span>
                <input type="number" v-model="withdrawAmount" required min="5000"
            ...
            <div>
              ...
              <div class="grid grid-cols-3 gap-2">
                <button type="button" @click="payoutMethod = 'momo_mtn'"
                ...
                <button type="button" @click="payoutMethod = 'momo_airtel'"
                  class="py-2.5 px-2 rounded-xl border font-bold text-xs flex flex-col items-center gap-1 transition-all cursor-pointer"
            ...
            <div v-if="payoutMethod === 'bank'">
              ...
              <input type="text" v-model="bankName" placeholder="e.g. Stanbic Bank Uganda"
                class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-[#024731] text-xs font-semibold" />
            </div>
            ...
            <div>
              ...
              <input type="text" v-model="accountName" placeholder="Full legal name"
                class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-[#024731] text-xs font-semibold" />
            </div>
