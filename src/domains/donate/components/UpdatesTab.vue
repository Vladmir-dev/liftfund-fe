  ...
  <div class="flex flex-col gap-6 animate-fade-in text-left">
    ...
    <div class="rounded-3xl bg-slate-100/60 border border-slate-200/50 p-6 flex flex-col gap-5">
      <p class="text-xs text-slate-650 font-semibold leading-relaxed">
        <span class="text-blue-700 font-bold block text-sm mb-1.5">Send fundraiser updates to your donors.</span>
      ...
      <div v-if="showAddUpdateInput" class="flex flex-col gap-3 animate-fade-in">
        <textarea v-model="updateText" placeholder="Write an update for your donors (e.g. We reached our first milestone!)..." class="w-full h-32 p-4 border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-600 rounded-xl text-xs font-medium resize-none"></textarea>
        <div class="flex justify-end gap-2.5">
          ...
          <button @click="postUpdate" :disabled="!updateText.trim()" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white font-bold text-xs rounded-lg cursor-pointer">Post update</button>
        </div>
      </div>

      <button v-else @click="showAddUpdateInput = true" class="inline-flex items-center gap-1.5 self-start bg-blue-600 hover:bg-blue-700 text-white font-extrabold text-xs px-5 py-3 rounded-full transition-all shadow-md cursor-pointer focus:outline-none font-sans">
