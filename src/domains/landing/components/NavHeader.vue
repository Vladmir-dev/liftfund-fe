    ...
    <div class="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between relative">
      ...
      <div class="flex items-center gap-6">
        ...
        <div class="relative" @mouseenter="activeDropdown = 'donate'" @mouseleave="activeDropdown = null">
          <button @click="toggleDropdown('donate')" class="flex items-center gap-1.5 text-sm font-semibold text-slate-700 hover:text-blue-600 transition-colors py-2 cursor-pointer">
            Donate
          ...
          <div v-if="activeDropdown === 'donate'" class="absolute left-0 mt-0 pt-2 w-72 sm:w-[480px] z-50">
            <div class="bg-white rounded-xl shadow-xl border border-slate-100 p-5 grid grid-cols-1 sm:grid-cols-2 gap-4 animate-scale">
              <div class="col-span-full border-b border-slate-50 pb-2 mb-1 flex items-center gap-2 text-slate-500 text-xs font-bold uppercase tracking-wider">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" class="text-blue-600"><path stroke-linecap="round" stroke-linejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z"/></svg>
              ...
              <RouterLink to="/donate/categories" @click="closeDropdowns" class="group flex flex-col p-2.5 rounded-lg hover:bg-slate-50 transition-colors">
                <span class="text-sm font-semibold group-hover:text-blue-600">Categories</span>
              ...
              <RouterLink to="/donate/crisis-relief" @click="closeDropdowns" class="group flex flex-col p-2.5 rounded-lg hover:bg-slate-50 transition-colors">
                <span class="text-sm font-semibold group-hover:text-blue-600">Crisis Relief</span>
              ...
              <RouterLink to="/donate/social-impact" @click="closeDropdowns" class="group flex flex-col p-2.5 rounded-lg hover:bg-slate-50 transition-colors">
                <span class="text-sm font-semibold group-hover:text-blue-600">Social Impact Funds</span>
              ...
              <RouterLink to="/donate/supporter-space" @click="closeDropdowns" class="group flex flex-col p-2.5 rounded-lg hover:bg-slate-50 transition-colors">
                <span class="text-sm font-semibold group-hover:text-blue-600">Supporter Space</span>
        ...
        <div class="relative hidden sm:block" @mouseenter="activeDropdown = 'fundraise'" @mouseleave="activeDropdown = null">
          <button @click="toggleDropdown('fundraise')" class="flex items-center gap-1.5 text-sm font-semibold text-slate-700 hover:text-blue-600 transition-colors py-2 cursor-pointer">
            Fundraise
          ...
          <div v-if="activeDropdown === 'fundraise'" class="absolute left-0 mt-0 pt-2 w-56 z-50">
            <div class="bg-white rounded-xl shadow-xl border border-slate-100 p-3 flex flex-col gap-1 animate-scale">
              <button @click="startFundraiser" class="w-full text-left px-3 py-2 rounded-lg text-sm font-semibold hover:bg-slate-50 hover:text-blue-600 transition-colors cursor-pointer">Start a Fundraiser</button>
              <a href="/#how-it-works" @click="closeDropdowns" class="w-full text-left px-3 py-2 rounded-lg text-sm font-semibold hover:bg-slate-50 hover:text-blue-600 transition-colors cursor-pointer">How HelpFund Works</a>
            </div>
          </div>
        </div>
      </div>
      ...
      <RouterLink to="/" @click="closeDropdowns" class="flex items-center gap-1 absolute left-1/2 transform -translate-x-1/2">
        ...
        <span class="text-xl font-black text-slate-800 tracking-tight">helpfund</span>
      </RouterLink>
      ...
      <div class="flex items-center gap-4">
        ...
        <div class="relative hidden lg:block" @mouseenter="activeDropdown = 'about'" @mouseleave="activeDropdown = null">
          <button @click="toggleDropdown('about')" class="flex items-center gap-1.5 text-sm font-semibold text-slate-700 hover:text-blue-600 transition-colors py-2 cursor-pointer">
            About
          ...
          <div v-if="activeDropdown === 'about'" class="absolute right-0 mt-0 pt-2 w-72 sm:w-[480px] z-50">
            <div class="bg-white rounded-xl shadow-xl border border-slate-100 p-5 grid grid-cols-1 sm:grid-cols-2 gap-4 animate-scale">
              <div class="col-span-full border-b border-slate-50 pb-2 mb-1 flex items-center gap-2 text-slate-500 text-xs font-bold uppercase tracking-wider">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" class="text-blue-600"><path stroke-linecap="round" stroke-linejoin="round" d="M11.25 11.25l.041-.02a.75.75 0 111.063.852l-.708 2.836a.75.75 0 001.063.852l.041-.021M21 12a9 9 0 11-18 0 9 9 0 0118 0zm-9-3.75h.008v.008H12V8.25z"/></svg>
              ...
              <a href="/#how-it-works" @click="closeDropdowns" class="flex flex-col p-2 hover:bg-slate-50 rounded-lg group">
                <span class="text-sm font-semibold group-hover:text-blue-600">How HelpFund works</span>
              </a>
              <RouterLink to="/donate/supporter-space" @click="closeDropdowns" class="flex flex-col p-2 hover:bg-slate-50 rounded-lg group">
                <span class="text-sm font-semibold group-hover:text-blue-600">About HelpFund</span>
              </RouterLink>
              <RouterLink to="/donate/crisis-relief" @click="closeDropdowns" class="flex flex-col p-2 hover:bg-slate-50 rounded-lg group">
                <span class="text-sm font-semibold group-hover:text-blue-600">HelpFund Giving Guarantee</span>
              </RouterLink>
              <RouterLink to="/donate/categories" @click="closeDropdowns" class="flex flex-col p-2 hover:bg-slate-50 rounded-lg group">
                <span class="text-sm font-semibold group-hover:text-blue-600">Supported Countries</span>
              </RouterLink>
              <RouterLink to="/donate/social-impact" @click="closeDropdowns" class="flex flex-col p-2 hover:bg-slate-50 rounded-lg group">
                <span class="text-sm font-semibold group-hover:text-blue-600">Pricing</span>
              </RouterLink>
              <RouterLink to="/donate/supporter-space" @click="closeDropdowns" class="flex flex-col p-2 hover:bg-slate-50 rounded-lg group">
                <span class="text-sm font-semibold group-hover:text-blue-600">Help Center</span>
              </RouterLink>
            </div>
          </div>
        </div>
        ...
        <template v-if="!authStore.isLoggedIn">
          <button @click="goToLogin" class="text-sm font-semibold text-slate-700 hover:text-blue-600 transition-colors py-2 px-3 cursor-pointer">Sign in</button>
          <button @click="startFundraiser" class="bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold py-2 px-4 rounded-full shadow-md shadow-blue-100 hover:shadow-lg transition-all cursor-pointer">Start a HelpFund</button>
        </template>
        <template v-else>
          <!-- User Dropdown Menu -->
